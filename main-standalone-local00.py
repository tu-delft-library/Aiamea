...
def _ollama_worker(prompt_json, result_queue):
...
def run_batch(settings, log_q, progress_q):
    ...
    log_q.put(
        f"Ollama: {OLLAMA_BASE_URL} | Model: {LOCAL_MODEL} | Hard timeout: {OLLAMA_TIMEOUT}s | Max tokens: {OLLAMA_MAX_TOKENS} | Thinking: off"
    )
    ...
    for i, pdf_file in enumerate(
        ...
        if json_path.exists():
            ...
            log_q.put(
                f"[{i}/{total}] Sending metadata to Ollama/Qwen3:8b"
            )
            ...
            try:
                ...
                log_q.put(
                    f"[{i}/{total}] ERROR (LOCAL MODEL) {pdf_file.name}: {e}"
                )
...
class App(tk.Tk):
    ...
    def start_run(self):
        ...
        try:
            ...
            self.log_status(
                f"Ollama connected: {LOCAL_MODEL}"
            )

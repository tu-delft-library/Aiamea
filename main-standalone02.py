...
# OCR FUNCTIONS
# ----------------------------
def ocr_with_tesseract(pdf_path, dpi):
...
def ocr_with_pymupdf(pdf_path, dpi=None):
    import fitz
...
def ocr_pdf(pdf_path, ocr_engine, dpi):
    if ocr_engine == "pymupdf":
        return ocr_with_pymupdf(pdf_path, dpi)
    elif ocr_engine == "tesseract":
        return ocr_with_tesseract(pdf_path, dpi)
    else:
...
def run_batch(settings, log_q, progress_q):
    ...
    for i, pdf_file in enumerate(files, start=1):
        ...
        if ocr_cache_path.exists():
            ...
            try:
                ocr_text = ocr_pdf(pdf_file, ocr_engine, dpi)
